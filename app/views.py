...
def available_building_rooms(request, building_id):
    ...
    :return:
    """
    rooms = Room.objects.filter(floor__building__id=building_id, is_occupied=False)
...
def get_pending_bookings_of_building(request, building_id):
    ...
    bookings_of_building = all_bookings.filter(room__floor__building__id=building_id, is_confirmed=False)
...
def get_confirmed_bookings_of_building(request, building_id):
    all_bookings = Booking.objects.all().order_by('-id')
    bookings_of_building = all_bookings.filter(room__floor__building__id=building_id, is_confirmed=True)
    serializer = BookingSerializer(bookings_of_building, many=True)
    return JsonResponse({"response": serializer.data}, status=200)
...
class BookingDetailAPIView(APIView):
    ...
    def patch(self, request, booking_id=None):
        ...
        incoming_data = request.data
        instance = self.get_object(booking_id)
        serializer = BookingSerializer(instance, data=incoming_data, partial=True)
...
def count_totals(request, building_id, key):
    if key == 'PENDING_BOOKING':
        ...
        all_bookings = Booking.objects.all().order_by('-id')
        total_confirmed_bookings = all_bookings.filter(room__floor__building__id=building_id, is_confirmed=True).count()
        return JsonResponse({"total": total_confirmed_bookings}, status=200)

    elif key == 'TENANTS':
        all_tenants = Tenant.objects.all().order_by('-id')
        total_tenants = all_tenants.filter(room__floor__building__id=building_id).count()
        return JsonResponse({"total": total_tenants}, status=200)

    elif key == 'PAID_BILLS':
        all_bills = Billing.objects.all().order_by('-id')
        total_bills = all_bills.filter(building__id=building_id, is_paid=True).count()
        return JsonResponse({"total": total_bills}, status=200)

    elif key == 'UNPAID_BILLS':
        all_bills = Billing.objects.all().order_by('-id')
        total_bills = all_bills.filter(building__id=building_id, is_paid=False).count()
        return JsonResponse({"total": total_bills}, status=200)
    else:
...
class BillingViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = (filters.SearchFilter,)
    search_fields = ('tenant__tenant_names', 'invoice_no')

    def get_queryset(self, building_id=None):
        building_id = self.request.query_params.get('building_id')
        queryset = Billing.objects.all().filter(building__id=building_id).order_by('-id')
        return queryset
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(sub_total=sub_total, tax_amount=tax_amount)
...
class TenantViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = (filters.SearchFilter,)
    search_fields = ('room__name', 'tenant_names', 'tenant_tin', 'business_desc')

    def get_queryset(self):
        building_id = self.request.query_params.get('building_id')
        queryset = Tenant.objects.all().filter(room__floor__building__id=building_id).order_by('-id')
        return queryset
